...
`main`へのマージは検証だけを実行します。Readyなメンバー変更PRではplanが自動実行されます。手動起動した`Apply`も初期状態では承認なしでplanだけを実行します。権限を反映するには、最新の`main`で`apply`をオンにして手動起動し、管理者が`production` Environmentで承認します。
