...
Readyなメンバー変更PRではplanが自動実行されます。手動でも、Applyの`apply`をオフにして実行できます。mainへのpushでApplyが起動し、`production` Environmentで承認を待ちます。適用を再試行する場合は、最新の`main`で`apply`をオンにして手動起動し、同じ承認を行います。

承認待ちの間に`main`が更新されると実行は停止します。古い実行をキャンセルし、最新の`main`の実行を確認してください。applyの承認待ちでもPRのplanは実行できます。手動で外された管理対象の権限も、次の適用で設定に合わせて戻します。定期実行は設定していません。
...
メンバーファイルを変更するPRでは、作成・再オープン・追加push・Ready for reviewへの変更時にplanを自動実行します。Draftでは実行しません。`pull_request_target`を使い、mainのコードと指定SHAのメンバーYAMLだけで実行します。PRのplanからapplyへ進むことはありません。
...
1. PRのtfcmtコメントで追加・変更・取り消しの差分を確認する。
2. マージ後に起動したApplyを開き、対象コミットを確認する。
3. 適用する場合はReview deploymentsから`production`を承認する。

PoC中は承認せず、planでの確認までに留めます。承認待ちの実行はキャンセルできます。

マージ後にもう一度planを確認したい場合は、最新の`main`で`pr_number`・`pr_sha`を空にし、`apply`をオフにして手動実行できます。

承認後に既存割り当てをGCSの本番stateへimportし、planを作り直します。import自体は割り当てを変更しませんが、本番stateは更新します。外部で権限が変更されると差分が変わる場合があるため、tfcmtコメントの対象コミットと実行リンクを確認してください。
